...
class BaoquanClientTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testApplyCa0() {
        ...
        $this->client->applyCa(new ApplyCaPayload());
    }
    ...
    public function testApplyCa3() {
        ...
        $payload->setLinkName($this->faker->name);
        $payload->setLinkIdCard($this->randomIdCard());
        $payload->setLinkPhone($this->faker->phoneNumber);
        $payload->setLinkEmail($this->faker->email);
        $this->client->applyCa($payload);
    }
    ...
    public function testApplyCa5() {
        ...
        $response = $this->client->applyCa($payload, $seal);
        $this->assertNotEmpty($response);
        $this->assertNotEmpty($response->getRequestId());
        $this->assertNotEmpty($response->getData());
        $this->assertNotEmpty($response->getData()->getNo());
    ...
    public function testSign1() {
        ...
        $attachment0->setResource(fopen(__DIR__.'/resources/contract.pdf', 'r'));
        $attachment0->setResourceName('contract.pdf');
        $attachment1 = new Attachment();
        $attachment1->setResource(fopen(__DIR__.'/resources/seal.png', 'r'));
        $attachment1->setResourceName('seal.png');
        $attachments = [
            0=>[$attachment0, $attachment1]
        ];
        $response = $this->client->createAttestation($payload, $attachments);
        $this->assertNotEmpty($response);
        $this->assertNotEmpty($response->getRequestId());
    ...
    public function testSign2() {
        ...
        $payload->setSigns($signs);
        $attachment0 = new Attachment();
        $attachment0->setResource(fopen(__DIR__.'/resources/seal.png', 'r'));
        $attachment0->setResourceName('seal.png');
        $attachment1 = new Attachment();
        $attachment1->setResource(fopen(__DIR__.'/resources/seal.png', 'r'));
        $attachment1->setResourceName('seal.png');
        $attachment2 = new Attachment();
        $attachment2->setResource(fopen(__DIR__.'/resources/contract.pdf', 'r'));
        $attachment2->setResourceName('contract.pdf');
        $attachments = [
            0=>[$attachment0],
            1=>[$attachment1, $attachment2]
        ];
        $response = $this->client->createAttestation($payload, $attachments);
        $this->assertNotEmpty($response);
        $this->assertNotEmpty($response->getRequestId());
    ...
    private function randomIdCard() {
        ...
        $sums = 0;
        for ($i=0;$i< 17;$i++){
            $sums += (int)substr($base,$i,1) * $factor[$i];
        }
        $mods = $sums % 11;//10X98765432
        switch ($mods){
            case 0:  $suffix_d = '1';break;
            case 1:  $suffix_d = '0';break;
            case 2:  $suffix_d = 'x';break;
            case 3:  $suffix_d = '9';break;
            case 4:  $suffix_d = '8';break;
            case 5:  $suffix_d = '7';break;
            case 6:  $suffix_d = '6';break;
            case 7:  $suffix_d = '5';break;
            case 8:  $suffix_d = '4';break;
            case 9:  $suffix_d = '3';break;
            case 10: $suffix_d = '2';break;
        }
        return $base.$suffix_d;
    }
}